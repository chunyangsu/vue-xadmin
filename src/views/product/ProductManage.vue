<template>
  <d2-container>
    ...
    <!-- 新建编辑弹窗 -->
    <product-dialog v-if="pro_update_show" ref="ProductDialog" :pro-title="proTitle" :cur-id="pro_id" @createBack="handleSearch" @updateBack="updateBack" />
  </d2-container>
</template>
  ...
  methods: {
    ...
    deleteData(row) {
      this.$confirm('此操作将删除该数据, 是否继续?', '提示', {
        ...
        .then(() => {
          this.$api.deleteProduct(row.id).then(response => {
            this.$message({
              type: 'success',
              message: '删除成功'
            })
